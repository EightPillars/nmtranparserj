...
$INPUT ID TIME DT WT D DVID DV MDV LOGTWT
$DATA ../data/warfarin_conc_analytic.csv IGNORE=#
...
;@MCL_$ERROR
   ...
   IF ((TIME-DT).LE.TLAG) THEN
      CC=0
   ELSE
      CC=D*KA/(V*(KA-K))*(EXP(-K*(TIME-DT-TLAG)) - EXP(-KA*(TIME-DT-TLAG)))
   ENDIF
